...
- Made image desired-state generation monotonic: an older run can no longer overwrite a newer queued image's source/source-key/position/cover/status metadata, while a newer enqueue preserves an active worker lease and is re-read under the existing queue row lock before persistence.
