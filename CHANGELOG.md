...
- Heartbeat every still-active sibling image lease owned by the same claim token while retaining exact current-row ownership verification, preventing slow downloads/attachments from expiring untouched siblings from the same claimed batch.
