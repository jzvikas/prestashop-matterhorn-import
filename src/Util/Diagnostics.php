...
final class Diagnostics
{
    ...
    public function __construct(private SourceInterface $source, private DatabaseSafety $databaseSafety) {}
    ...
    public function run(int $shopId): array
    {
        ...
        try {
            $this->databaseSafety->assertTransactionalCore();
            $checks[] = $this->check('core-db-engine', 'ok', 'critical tables are InnoDB');
        } catch (\Throwable $e) {
            $checks[] = $this->check('core-db-engine', 'error', $e->getMessage());
        }
        ...
        try {
            if ($this->source instanceof CheckpointableSourceInterface) {
                $checks[] = $this->check('source', 'ok', $this->source->name() . ' fingerprint=' . substr($this->source->fingerprint(), 0, 12));
            } else {
                $checks[] = $this->check('source', 'warning', $this->source->name() . ' has no fingerprint check');
            }
        } catch (\Throwable $e) {
            $checks[] = $this->check('source', 'error', $e->getMessage());
        }
