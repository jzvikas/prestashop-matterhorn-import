...
final class CommandInput
{
    ...
    public static function workerLabel(mixed $value): string
    {
        if (!is_string($value)) { throw new \InvalidArgumentException('--worker must be a string'); }
        $value = trim($value);
        if ($value === '' || strlen($value) > 80 || preg_match('/^[A-Za-z0-9_.:-]+$/D', $value) !== 1) {
            throw new \InvalidArgumentException('--worker must be 1-80 safe label characters');
        }
        return $value;
    }
    private static function parseInt(mixed $value, string $name, int $min, int $max): int
    {
        if (is_int($value)) { $parsed = $value; }
            ...
            if ($parsed === false) { throw new \InvalidArgumentException($name . ' must be an integer between ' . $min . ' and ' . $max); }
        }
        if ($parsed < $min || $parsed > $max) { throw new \InvalidArgumentException($name . ' must be an integer between ' . $min . ' and ' . $max); }
        return (int) $parsed;
    }
}
