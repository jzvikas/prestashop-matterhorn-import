...
final class MatterhornXmlSource implements ByteCheckpointableSourceInterface
{
    ...
    private function stream(int $byteOffset, int $skipRecords, int $recordOffset): iterable
    {
        ...
        $readBytes = 0;
        // StringWalker is Prewk's XML-aware parser. Unlike UniqueNode it tracks
        // XML/CDATA boundaries instead of searching for a literal </product>, so a
        // supplier description containing that text cannot truncate the product.
        // A fresh document starts below <products> (depth 2); a byte-resume starts
        // exactly between products and therefore captures the next node at depth 1.
