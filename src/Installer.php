...
final class Installer
{
    ...
    public function install(): bool
    {
        ...
        try {
            ...
            if (!$this->upgradeMappingState()) { throw new \RuntimeException('Could not initialize Matterhorn mapping state schema'); }
            $defaults = [
                self::RETAIN_DATA_KEY => '1','MATTERHORNIMPORT_CATEGORY_AUTO_CREATE' => '1','MATTERHORNIMPORT_FEATURE_AUTO_CREATE' => '1',
                'MATTERHORNIMPORT_SIZE_ATTRIBUTE_GROUP_NAME' => 'Size','MATTERHORNIMPORT_MAX_REMOVE_PERCENT' => '25',
            ];
            foreach ($defaults as $key => $value) {
                if (!\Configuration::updateValue($key, $value, false, 0, 0)) { throw new \RuntimeException('Could not initialize Matterhorn configuration: ' . $key); }
            }
            return true;
        } catch (\Throwable) {
            try { $this->uninstallSchemaOnly(); } catch (\Throwable) {}
            foreach (array_keys($defaults) as $key) { try { \Configuration::deleteByName($key); } catch (\Throwable) {} }
            return false;
        }
    }
    ...
    public function upgradeMappingState(): bool
    {
        try {
            ...
            $column = $db->getRow("SHOW COLUMNS FROM `" . str_replace('`','``',$table) . "` LIKE 'out_of_feed'");
            if (!is_array($column)) {
                if (!$db->execute("ALTER TABLE `" . str_replace('`','``',$table) . "` ADD COLUMN `out_of_feed` TINYINT(1) NOT NULL DEFAULT 0 AFTER `image_hash`")) {
                    throw new \RuntimeException('Could not add Matterhorn out_of_feed mapping state: ' . $db->getMsgError());
                }
            }
            $index = $db->getRow("SHOW INDEX FROM `" . str_replace('`','``',$table) . "` WHERE Key_name='idx_feed_state'");
            if (!is_array($index)) {
                if (!$db->execute("ALTER TABLE `" . str_replace('`','``',$table) . "` ADD KEY `idx_feed_state` (`id_shop`,`source`,`out_of_feed`,`last_seen_run_id`)")) {
    ...
    public function uninstall(): bool
    {
        ...
        if (!$retainData && !$this->uninstallSchemaOnly()) { return false; }
        $ok = true;
        foreach (self::CONFIG_KEYS as $key) { $ok = \Configuration::deleteByName($key) && $ok; }
        return $ok;
    }
    ...
    private function uninstallSchemaOnly(): bool
    {
        foreach (self::UNINSTALL_SQL as $file) {
            foreach ($this->statements($file) as $sql) { if (!\Db::getInstance()->execute($sql)) { return false; } }
        }
        return true;
    }
