...
final class SnapshotRepository
{
    ...
    /** @return list<array{source_key:string,payload:string,id_product:int}> */
    public function imageManifestRows(int $runId, int $shopId, string $source, string $after = '', int $limit = 500): array
    {
        ...
        $where = sprintf(
            " FROM `%s%s` s INNER JOIN `%s%s` m ON m.id_shop=%d AND m.source='%s' AND m.source_key=s.source_key WHERE s.id_run=%d AND m.out_of_feed=0%s",
            _DB_PREFIX_, self::TABLE, _DB_PREFIX_, self::MAPPING_TABLE, $shopId, pSQL($source), $runId, $cursor
        );
        $window = $this->payloadWindow(
            'SELECT s.source_key,OCTET_LENGTH(s.payload) payload_bytes' . $where . ' ORDER BY s.source_key LIMIT ' . $limit,
            'source_key'
        );
        if ($window === null) {
            return [];
