...
final class ImageStateRepository
{
    ...
    private const MAX_STALE_SCAN_ROWS = 50000;
    ...
    public function staleSourceKeys(int $shopId, string $source, int $ageHours = 24, int $limit = 500): array
    {
        ...
        $scanLimit = min(self::MAX_STALE_SCAN_ROWS, max($limit, $limit * 10));
        $rows = \Db::getInstance()->executeS(sprintf(
            "SELECT s.source_key FROM `%s%s` s " .
            ...
            "AND NOT EXISTS (SELECT 1 FROM `%sli_matterhornim_99dfbf_image_queue` q WHERE q.id_shop=s.id_shop AND q.source=s.source AND q.source_key=s.source_key AND q.id_product=s.id_product AND q.status<>'done') " .
    ...
    public function touchNotModified(array $queueRow, int $idImage): void
    {
        ...
        if ($rows === false) { throw new \RuntimeException('Image state revalidation lock query failed: ' . $db->getMsgError()); }
        if (($rows[0] ?? null) === null) { throw new \RuntimeException('Image state revalidation lost its live PrestaShop image association'); }
    }
    ...
    public function save(array $queueRow, int $idImage, DownloadedImage $download): void
    {
        ...
        if (!\Db::getInstance()->execute($sql)) { throw new \RuntimeException('Image state save failed'); }
    }
    ...
    public function deleteState(int $shopId, string $source, string $sourceKey, int $productId, string $urlHash): void
    {
        ...
        if (!\Db::getInstance()->delete(self::TABLE, $where)) { throw new \RuntimeException('Image state delete failed'); }
    }
