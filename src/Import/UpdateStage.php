...
final class UpdateStage
{
    ...
    public function run(int $runId, int $shopId, string $source, int $batch = 500, int $maxItems = 0, int $timeLimitSeconds = 0): bool
    {
        ...
        try {
            ...
            $cursor = 0; $failures = 0; $paused = false; $batch = max(1, min(2000, $batch));
            ...
            while (!$this->budget->shouldStop() && ($rows = $this->snapshots->changedRows($runId, $shopId, $source, $cursor, $batch)) !== []) {
                ...
                try {
                    foreach ($rows as $row) {
                        ...
                        try {
                            ...
                            $writerDomains = array_values(array_intersect($domains, ['core','price','stock','category']));
                ...
                } catch (\Throwable $batchError) { $db->execute('ROLLBACK'); throw $batchError; }
            ...
            if ($paused || $this->budget->shouldStop()) {
                $this->runs->stage($runId, 'update', 'paused'); $this->runs->finish($runId, 'paused'); return false;
            }
    ...
    {
        if (!$this->transactionIsActive($db)) { return; }
        if (!$db->execute('ROLLBACK TO SAVEPOINT ' . self::SAVEPOINT)) { $db->execute('ROLLBACK'); throw new \RuntimeException('Could not roll back UPDATE item savepoint', 0, $cause); }
    ...
    {
        $domains = [];
        foreach (['core','price','stock','feature','category'] as $domain) {
