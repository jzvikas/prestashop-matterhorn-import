...
{
    public function __construct(private SourceInterface $source, private RunRepository $runs, private ReadStage $stage, private ImportLock $lock, private OperationalSettings $settings)
    {
    ...
    protected function configure(): void
    {
        $this->setDescription('Build or resume the Matterhorn normalized staging snapshot')
            ...
            ->addOption('max-items', null, InputOption::VALUE_REQUIRED, 'Maximum source rows; omitted = shop setting')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            if ($requestedRun !== null) { $runId = $requestedRun; $this->runs->assertContext($runId, $shop, $source); } else { $runId = $this->runs->create($shop, $source); }
            ...
            else { $output->writeln($completed ? '<info>READ completed for run #' . $runId . '</info>' : '<comment>READ paused safely for run #' . $runId . '; resume with --run=' . $runId . '</comment>'); }
