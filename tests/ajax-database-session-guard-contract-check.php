...
$check(str_contains($guard, 'WAIT_TIMEOUT_SECONDS = 300'), 'DB wait_timeout guard must be 300 seconds');
$check(str_contains($guard, 'NET_READ_TIMEOUT_SECONDS = 120'), 'DB net_read_timeout guard missing');
$check(str_contains($guard, 'NET_WRITE_TIMEOUT_SECONDS = 120'), 'DB net_write_timeout guard missing');
...
$check(str_contains($guard, 'use Doctrine\\DBAL\\Exception\\ConnectionLost;'), 'Doctrine ConnectionLost type classifier missing');
$check(str_contains($guard, '$current instanceof ConnectionLost'), 'Doctrine ConnectionLost must be detected by type');
...
$check(str_contains($guard, '$db->getNumberError()'), 'legacy DB numeric error detection missing');
$check(str_contains($guard, '$this->legacyConnectionLost($db)'), 'legacy false-return connection loss recovery missing');
...
$check(str_contains($guard, '2006 || $number === 2013'), 'legacy MySQL 2006/2013 classifier missing');

$check(str_contains($subscriber, "ROUTE_PREFIX = 'matterhorn_import_'"), 'DB guard must cover every Matterhorn BO route');
$check(!str_contains($subscriber, "ROUTE_PREFIX = 'matterhorn_import_ajax'"), 'DB guard must not be limited to AJAX routes');
