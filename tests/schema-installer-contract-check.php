...
$performanceIndexes = (string) file_get_contents($root . '/sql/performance-indexes.sql');
...
schemaCheck(substr_count($performanceIndexes, 'ALTER TABLE') === 4, 'performance schema must define exactly four targeted indexes');
...
schemaCheck(!str_contains($performanceIndexes, 'PREFIX_lp_import_'), 'generic skeleton table token must not leak into performance indexes');
...
schemaCheck(str_contains($performanceIndexes, 'idx_shop_source_run'), 'latest-run hot query index missing');
schemaCheck(str_contains($performanceIndexes, 'idx_feed_product'), 'REMOVE keyset hot query index missing');
schemaCheck(substr_count($performanceIndexes, 'idx_shop_claim') === 2, 'both per-shop worker queues need claim indexes');
...
schemaCheck(str_contains($installer, "'performance-indexes.sql'"), 'installer must load high-volume performance indexes');
...
schemaCheck(str_contains($upgrade013, 'INFORMATION_SCHEMA.STATISTICS'), '0.1.3 index upgrade must be idempotent');
schemaCheck(str_contains($upgrade013, 'idx_feed_product') && str_contains($upgrade013, 'idx_shop_source_run'), '0.1.3 upgrade must add hot-query indexes');
